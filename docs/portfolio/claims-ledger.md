...
| Local queue-mode load tests show worker separation and N/N worker participation under concurrent mock-only inbound load, with non-linear host-bound throughput. | `docs/load-test-findings.md`; `load/k6/`; `scripts/run-load-tests.ps1`; `scripts/smoke-load-k8s.ps1` | Single-machine, mock-only worksheet. Not a production SLA/HA claim, not real Gemini/GHL throughput, not linear scaling. Participation proves processing presence, not balanced per-worker distribution. At the heaviest local tier 2 workers beat 4 workers; the 4-worker inversion must not be hidden. The isolated kind snapshot is a deployment-under-load check, not a Compose comparison. |
